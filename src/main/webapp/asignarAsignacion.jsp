...
<body>
    ...
    <form action="AsignarAsignacionController" method="post">
        ...
        <select name="idEmpleado" id="empleados">
            <% List<Empleado> empleados = BaseDeDatos.obtenerTodosLosEmpleados(); %>
            <% for (Empleado empleado : empleados) { %>
            <option value="<%= empleado.getIdEmpleado() %>" <%= (empleado.getIdEmpleado() == empleadoSesion.getIdEmpleado()) ? "selected" : "" %>>
                <%= empleado.getNombreCompleto() %></option>
            <% } %>
        </select> 
        <br> 
        <label for="asignaciones">Seleccionar Asignación:</label> 
        <select name="idAsignacion" id="asignaciones">
