...
<body>
    ...
    <form action="AltaAsignacionController" method="post">
        ...
        <select name="nombreUsuario" id="empleados">
            <% List<Empleado> empleados = BaseDeDatos.obtenerTodosLosEmpleados(); %>
            <% for (Empleado empleado : empleados) { %>
            <option value="<%= empleado.getNombreUsuario() %>" <%= (empleado.getNombreUsuario().equals(empleadoSesion	.getNombreUsuario())) ? "selected" : "" %>>
                <%= empleado.getNombreCompleto() %></option>
            <% } %>
        </select> 
       <br> 
        <label for="fch_inicio">Fecha de Inicio:</label> 
        <input type="date" name="fch_inicio" value="<%=java.time.LocalDate.now().minusDays(1)%>" required>
        <br> 
        <label for="fch_fin">Fecha de Fin:</label> 
        <input type="date" name="fch_fin" value="<%=java.time.LocalDate.now()%>" min="<%=java.time.LocalDate.now().minusDays(1)%>" required>
        <br> 
        <label for="nombre_asignacion">Nombre de Asignación:</label> 
        <input type="text" name="nombre_asignacion" required>
        <br>
        <label for="detalle">Detalle:</label>
        <textarea name="detalle" rows="4" cols="50" required></textarea>
       <br>
        <label for="id_req">ID Requerimiento:</label>
        <select name="id_req">
        ...
        <br>
        <label for="id_caso">ID Caso de Uso:</label>
        <select name="id_caso">
        ...
        <br> 
        <input type="submit" value="Crear Asignación"> 
